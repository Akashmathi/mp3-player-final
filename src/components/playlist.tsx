...
function Row({ item, index, move, onPlay, onDelete }: RowProps) {
  ...
  return (
    <div ref={ref} className={`playlist-item ${item.active ? 'active' : ''}`} style={{ opacity: isDragging ? 0.5 : 1 }}>
      <div className="flex items-center gap-3 flex-1 min-w-0">
        <GripVertical size={16} className="text-muted-foreground cursor-grab shrink-0" />
        {item.thumbnail ? (
          <img src={item.thumbnail} alt="" className="h-10 w-10 rounded object-cover shadow-sm shrink-0" />
        ) : (
          <div className="h-10 w-10 rounded bg-accent flex items-center justify-center shrink-0">
            <Play size={16} className="text-muted-foreground" />
          </div>
        )}
        <div className="playlist-item-name truncate">
      ...
      <div className="flex gap-1 shrink-0 ml-2">
        ...
        <button className="btn icon-btn hover:text-destructive" onClick={() => onDelete(item.id)}>
          <Trash2 size={16} />
        </button>
      </div>
    </div>
