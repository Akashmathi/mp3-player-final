...
export function tx<T = unknown>(db: IDBDatabase, store: string, mode: IDBTransactionMode, op: (store: IDBObjectStore) => IDBRequest<T>) { 
...
export async function addFiles(files: File[]): Promise<StoredTrack[]> { 
  ...
  await Promise.all(toAdd.map((item) => tx(db, STORE_TRACKS, "readwrite", (s) => s.add(item)))); 
...
export type TrackUI = { id: string; name: string; url: string; size: number; createdAt: number; yt_id?: string; thumbnail?: string; };
...
export async function materializeTracksWithURLs(): Promise<TrackUI[]> { 
  ...
  const ordered = (order.length ? order : tracks.map((t) => t.id)).map((id) => map.get(id)).filter(Boolean) as StoredTrack[]; 
...
export async function addYouTubeTrack(data: { title: string; artist: string; yt_id: string; thumbnail: string }): Promise<StoredTrack> {
  const db = await openDB();
  ...
  await tx(db, STORE_TRACKS, "readwrite", (s) => s.add(track));
  const existingOrder = (await getOrder().catch(() => [])) as string[];
  await setOrder([...existingOrder, track.id]);
  return track;
}
